...
public class TodosService {
    ...
    public Todos getTodoById(Integer id) {
        Todos todos = repository.findById(id).orElseThrow(() -> {
            throw new ItemNotFoundException(HttpStatus.NOT_FOUND, "Todo not found: Invaid Todo Id");
        });
        return todos;
    }
    ...
    public List<Todos> getTodosByUserId(Integer id) {
        ...
        if (todos.size() == 0) {
            throw new ItemNotFoundException(HttpStatus.NOT_FOUND, "There is not Todo for provided User Id");
        }
        return todos;
    }
}
